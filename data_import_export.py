...
def read_csv(file):    
    ...
    if path.exists(file) == True:             
        ...
        raise FileNotFoundError(f'(data_import) Datei "{file}" nicht im \
...
def import_data(data,table,databasename):    
    ...
    if path.exists(f'{databasename}.db') == True:    
        ...
        if len(result.all()) == 0:
           ...
           raise LookupError(f'(data_import) Es sind bereits Daten in der Tabelle \
                             "{table}" vorhanden. Daher werden keine neuen Daten importiert.')
          
    else:
        raise FileNotFoundError(f'(data_import) Datebank-Datei "{databasename}" \
                                nicht im Verzeichnis gefunden.')